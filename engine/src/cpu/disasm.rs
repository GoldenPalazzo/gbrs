...
impl Reg16 {
    pub fn extract(opcode: u8, kind: Reg16Kind) -> Self {
        match (opcode >> 4) & 3 {
            ...
            1 => Self::DE,
            2 => match kind {
                Reg16Kind::Normal | Reg16Kind::Stk => Self::HL,
...
impl Instruction {
    ...
    pub fn decode(opcode: u8) -> Option<Self> {
        match opcode {
            0x00 => Some(Self::NOP),
            0x03 | 0x13 | 0x23 | 0x33 => Some(Self::INC(Operand::Reg16(Reg16::extract(
                opcode,
                Reg16Kind::Normal,
            )))),
            0x04 | 0x0c | 0x14 | 0x1c | 0x24 | 0x2c | 0x34 | 0x3c => {
                Some(Self::INC(Operand::Reg8(Reg8::extract(opcode, 3))))
            }
            0x05 | 0x0d | 0x15 | 0x1d | 0x25 | 0x2d | 0x35 | 0x3d => {
                Some(Self::DEC(Operand::Reg8(Reg8::extract(opcode, 3))))
            }
            0x0b | 0x1b | 0x2b | 0x3b => Some(Self::DEC(Operand::Reg16(Reg16::extract(
                opcode,
                Reg16Kind::Normal,
            )))),
            0x09 | 0x19 | 0x29 | 0x39 => Some(Self::ADD(
                Operand::Reg16(Reg16::HL),
                Operand::Reg16(Reg16::extract(opcode, Reg16Kind::Normal)),
            )),

            0x06 | 0x0e | 0x16 | 0x1e | 0x26 | 0x2e | 0x36 | 0x3e => Some(Self::LD(
                Operand::Reg8(Reg8::extract(opcode, 3)),
                Operand::Imm8,
            )),

            0x08 => Some(Self::LD(Operand::AddrDirect16, Operand::Reg16(Reg16::SP))),
            ...
            0x20 | 0x28 | 0x30 | 0x38 => {
                Some(Self::JR(Some(Condition::extract(opcode)), Operand::Imm8))
            }
            ...
            0x80..=0x87 => Some(Self::ADD(
                Operand::Reg8(Reg8::A),
                Operand::Reg8(Reg8::extract(opcode, 0)),
            )),
            ...
            0xc0 | 0xc8 | 0xd0 | 0xd8 => Some(Self::RET(Some(Condition::extract(opcode)))),
            0xc9 => Some(Self::RET(None)),
            0xd9 => Some(Self::RETI),

            0xc2 | 0xca | 0xd2 | 0xda => {
                Some(Self::JP(Some(Condition::extract(opcode)), Operand::Imm16))
            }

            0xc3 => Some(Self::JP(None, Operand::Imm16)),

            0xe9 => Some(Self::JP(None, Operand::Reg16(Reg16::HL))),

            0xc4 | 0xcc | 0xd4 | 0xdc => {
                Some(Self::CALL(Some(Condition::extract(opcode)), Operand::Imm16))
            }
            ...
            0xe0 => Some(Self::LDH(Operand::AddrDirectLow8, Operand::Reg8(Reg8::A))),
            0xea => Some(Self::LD(Operand::AddrDirect16, Operand::Reg8(Reg8::A))),
            0xf2 => Some(Self::LDH(
                Operand::Reg8(Reg8::A),
                Operand::AddrIndirectLow8(Reg8::C),
            )),
...
mod tests {
    ...
    fn test_disasm() {
        let cases = vec![
            (vec![0x00], Some(Instruction::NOP)),
            (
                vec![0x23],
                Some(Instruction::INC(Operand::Reg16(Reg16::HL))),
            ),
            (vec![0x7], Some(Instruction::RLCA)),
            (
                vec![0x38, 0x05],
                Some(Instruction::JR(Some(Condition::Carry), Operand::Imm8)),
            ),
            (vec![0x2d], Some(Instruction::DEC(Operand::Reg8(Reg8::L)))),
            (
                vec![0x72],
                Some(Instruction::LD(
            ...
            (vec![0x76], Some(Instruction::HALT)),
            (
                vec![0xa9],
            ...
            (
                vec![0xc6, 0x08],
                Some(Instruction::ADD(Operand::Reg8(Reg8::A), Operand::Imm8)),
            ),
        ];
